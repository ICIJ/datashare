...
public class ProgressSmoother implements BiFunction<String, Double, Void> {
    ...
    public Void apply(String taskId, Double progress) {
        ...
        lastPublished.compute(taskId, (tId, lastUpdated) -> this.progressWithRate(tId, lastUpdated, now, progress));
        if (progress == 1.0) {
    ...
    private Long progressWithRate(final String taskId, final Long lastUpdated, final long now,
