...
public abstract class TemporalWorkflowImpl implements TemporalWorkflow {
    ...
    public void progress(ProgressSignal progressSignal) {
        logger.info("Received progressSignal for activity {}, currentProgress : {}", progressSignal.activityId(),
                    progressSignal.progress());
        currentProgress = progressSignal.progress();
        Workflow.upsertTypedSearchAttributes(PROGRESS_CUSTOM_ATTRIBUTE.valueSet(currentProgress));
    }
