...
public class ObjectAssert<T> extends GenericAssert<ObjectAssert<T>, T> {
    ...
    public ObjectAssert<T> hasNoGetterWithBlankValues() {
        if (actual == null) {
            fail("Actual object cannot be null");
        }
        Class<?> clazz = actual.getClass();
        for(Method method : clazz.getMethods()) {
            if(isGetter(method)){
                try {
                    Object actualValue = method.invoke(actual);
                    if(actualValue == null){
                        fail(String.format("Getter <%s> returned null", method.getName()));
                    }
                    if("".equals(actualValue)) {
                        fail(String.format("Getter <%s> returned empty String", method.getName()));
                    }
                } catch (Exception e) {
                    fail(String.format("Failed to invoke getter <%s>: %s", method.getName(), e.getMessage()));
                }
