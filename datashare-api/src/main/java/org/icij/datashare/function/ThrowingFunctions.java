...
public class ThrowingFunctions {
    ...
    public static final ThrowingFunction<String, ThrowingFunction<List<?>, String>> join =
            sep -> list -> String.join(sep, list.stream().map(Object::toString).collect(Collectors.toList()));
    ...
    public static final ThrowingFunction<String, ThrowingFunction<String, List<String>>> split =
            val -> str -> Arrays.asList(str.split(val));
    ...
    public static final ThrowingFunction<String, ThrowingFunction<String, String>> removePattFrom =
            pttrn -> str -> str.replaceAll(pttrn, "");
    ...
    public static final ThrowingFunction<List<String>, List<Integer>> parseInts =
            ints -> ints.stream().map(parseInt).collect(Collectors.toList());
    public static final ThrowingFunction<List<String>, List<Boolean>> parseBooleans =
            ints -> ints.stream().map(parseBoolean).collect(Collectors.toList());
    // Filter list
    public static final ThrowingFunction<Predicate<String>, ThrowingFunction<List<String>, List<String>>>
            filterElements = pred -> list -> list.stream().filter(pred).collect(Collectors.toList());
    ...
    public static <T> Optional<T> getProperty(String key, Properties properties,
