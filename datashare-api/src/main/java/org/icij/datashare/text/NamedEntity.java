...
public final class NamedEntity implements Entity {
    ...
    public enum Category implements Serializable {
        PERSON("PERS"), ORGANIZATION("ORG"), LOCATION("LOC"), EMAIL("MAIL"), DATE("DATE"), MONEY("MON"), NUMBER("NUM"),
        NONE("NONE"), UNKNOWN("UNK");
        ...
        public static Category parse(String entityCategory) {
            if (entityCategory == null || entityCategory.isEmpty() || entityCategory.trim().equals("0") ||
                entityCategory.trim().equals("O")) {
                return NONE;
            }
            try {
                ...
                for (Category cat : values()) {
                    ...
                    if (normEntityCategory.equalsIgnoreCase(catAbbreviation) || normEntityCategory.equalsIgnoreCase(
                            catAbbreviation.substring(0, min(catAbbreviation.length(), 3))))
                        return cat;
                }
                return UNKNOWN;
            }
        ...
        public static ThrowingFunction<List<String>, List<Category>> parseAll =
                list -> list.stream().map(Category::parse).filter((Category cat) -> cat != UNKNOWN)
                            .collect(Collectors.toList());
    }

    public static NamedEntity create(Category cat, String mention, List<Long> offsets, String doc, String rootDocument,
                                     Pipeline.Type extr, Language extrLang) {
        return new NamedEntity(cat, mention, offsets, doc, rootDocument, extr, extrLang, false, null, null);
    }

    public static NamedEntity create(Category cat, String mention, List<Long> offsets, String doc, String rootDocument,
                                     Pipeline.Type extr, Language extrLang, Map<String, Object> metadata) {
    ...
    public static List<NamedEntity> allFrom(String text, Annotations annotations) {
        return annotations.getTags().stream().map(tag -> from(text, tag, annotations))
                          .filter(ne -> ne.category != UNKNOWN).collect(Collectors.toList());
    }
    ...
    public static NamedEntity from(String text, NlpTag tag, Annotations annotations) {
        ...
        return NamedEntity.create(tag.getCategory(), mention, List.of((long) tag.getBegin()), annotations.documentId,
                                  annotations.rootId, annotations.pipelineType, annotations.language);
    }

    @JsonCreator
    private NamedEntity(@JsonProperty("category") Category category, @JsonProperty("mention") String mention,
                        @JsonProperty("offsets") List<Long> offsets, @JsonProperty("documentId") String documentId,
                        ...
                        @JsonProperty("isHidden") Boolean hidden, @JsonProperty("partsOfSpeech") String partsOfSpeech,
        ...
        this.id = DEFAULT_DIGESTER.hash(
                String.join("|", documentId, String.valueOf(offsets), extractor.toString(), mentionNorm));
    ...
    public String toString() {
        return "NamedEntity{" + "mention='" + mention + '\'' + ", id='" + id + '\'' + ", category=" + category +
               ", offsets=" + offsets + '}';
    }
