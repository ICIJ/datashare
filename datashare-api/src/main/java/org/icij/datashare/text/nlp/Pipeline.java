...
public interface Pipeline {
    ...
    enum Type implements EnumTypeToken {
        TEST((short) -1), CORENLP((short) 0), OPENNLP((short) 4), EMAIL((short) 5), SPACY((short) 6);
    ...
    enum Property {
        ENTITIES, CACHING, LANGUAGE, ENCODING;

        public String getName() {
            return name().toLowerCase().replace('_', '-');
        }

        public static Function<List<NamedEntity.Category>, Function<Boolean, Properties>> build =
                entityCategories -> enableCaching -> {
