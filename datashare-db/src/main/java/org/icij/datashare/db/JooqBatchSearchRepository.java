    ...
    public boolean save(final User user, final BatchSearch batchSearch) throws SQLException {
        try (Connection conn = connectionProvider.acquire()) {
            return using(conn, dialect).transactionResult(configuration -> {
                ...
                inner.insertInto(table(BATCH_SEARCH), field("uuid"), field("name"), field("description"), field("user_id"), field("prj_id"), field("batch_date")).
                        values(batchSearch.uuid, batchSearch.name, batchSearch.description, user.id, batchSearch.project.getId(), new Timestamp(batchSearch.getDate().getTime())).execute();
