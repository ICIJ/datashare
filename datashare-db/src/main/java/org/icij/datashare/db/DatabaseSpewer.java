...
public class DatabaseSpewer extends Spewer {
    ...
    @Override
    protected void writeDocument(TikaDocument tikaDocument, TikaDocument parent, TikaDocument root, int level) throws
            IOException {
        ...
        String contentType =
                ofNullable(tikaDocument.getMetadata().get(CONTENT_TYPE)).orElse(DEFAULT_VALUE_UNKNOWN).split(";")[0];
        ...
        Document document =
                DocumentBuilder.createDoc().with(project).withId(tikaDocument.getId()).with(tikaDocument.getPath())
                               ...
                               .with(languageGuesser.guess(content, tikaDocument.getPath()))
