...
public class JooqBatchSearchRepositoryTest {
    ...
    public void test_get_records_with_query_and_field() {
        ...
        BatchSearch batchSearch4 = new BatchSearch(asList(project("prj"),project("anotherPrj")),"qux","baz",asSet("q1", "q3"), new Date(currentDateTime + 50), State.SUCCESS);
        repository.save(batchSearch1);
        repository.save(batchSearch2);
        ...
        assertThat(repository.getRecords(User.local(), asList("prj", "otherPrj", "anotherPrj"), new BatchSearchRepository.WebQuery(singletonList("anotherPrj"), null, null))).hasSize(1);
        assertThat(repository.getRecords(User.local(), asList("prj", "otherPrj", "anotherPrj"), new BatchSearchRepository.WebQuery(asList("prj","anotherPrj"), null, null))).hasSize(3);

        assertThat(repository.getRecords(User.local(), asList("prj", "otherPrj", "anotherPrj"), new BatchSearchRepository.WebQuery(null, asList(String.valueOf(currentDateTime + 10), String.valueOf(currentDateTime + 100)), null))).hasSize(1);

        assertThat(repository.getRecords(User.local(), asList("prj", "otherPrj", "anotherPrj"), new BatchSearchRepository.WebQuery(null, null, singletonList(State.SUCCESS.toString())))).hasSize(1);
        assertThat(repository.getRecords(User.local(), asList("prj", "otherPrj", "anotherPrj"), new BatchSearchRepository.WebQuery(null, null, asList(State.SUCCESS.toString(), State.QUEUED.toString())))).hasSize(4);

        assertThat(repository.getRecords(User.local(), asList("prj", "otherPrj", "anotherPrj"), new BatchSearchRepository.WebQuery(asList("otherPrj","prj"), null, asList(State.SUCCESS.toString(), State.QUEUED.toString())))).hasSize(2);
    }
    ...
    public void test_get_records_with_query() {
        ...
        List<BatchSearchRecord> from0To2OrderByName = repository.getRecords(User.local(), asList("prj1", "prj2"), new BatchSearchRepository.WebQuery(1, 1, "name", "asc", "*", "all", null, null, null, null));
