...
public class DocumentUserRecommendationResource {
    ...
    @Operation(description = "Gets all user's document recommendations.", parameters = {
            @Parameter(name = "from", description = "if not provided it starts from 0", in = ParameterIn.QUERY),
            @Parameter(name = "size", description = "if not provided, the 50 first record from the \"from\" parameter",
                    in = ParameterIn.QUERY), @Parameter(name = "project",
            description = "if not provided, return every recommendations for every project", in = ParameterIn.QUERY)})
    @ApiResponse(responseCode = "200", description = "returns the user's document recommendations",
    ...
    public Payload get(Context context) {
        ...
        List<Project> projects = getUserProjects(context.currentUser()).stream()
        ...
        try {
            ...
            List<DocumentUserRecommendation> recommendations =
                    repository.getDocumentUserRecommendations(from, size, projects);
