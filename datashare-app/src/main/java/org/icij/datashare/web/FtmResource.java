...
public class FtmResource {
    ...
    @Operation(
            description = "Get the [FtM](https://followthemoney.tech/) document from its project and id (content hash)")
    ...
    @Get("/:project/:docId")
    public FtmDocument getDocument(
            @Parameter(name = "project", description = "project identifier", in = ParameterIn.PATH) String project,
        ...
        return notFoundIfNull(
                ofNullable(indexer.get(project, docId)).map(d -> new FtmDocument((Document) d)).orElse(null));
    }
