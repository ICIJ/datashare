...
public class RootResource {
    ...
    public String getRoot(Context context) throws IOException {
        ...
        if (context.env().classPath() && !index.toFile().isFile()) {
            ...
            copy(getClass().getResourceAsStream("/" + context.env().appFolder() + "/" + INDEX_HTML), writer,
                 Charset.defaultCharset());
            content = writer.toString();
        } else {
            content = new String(Files.readAllBytes(index), Charset.defaultCharset());
        }
        List<String> projectNames = context.currentUser() == null ? new LinkedList<>() :
                                    ((DatashareUser) context.currentUser()).getProjectNames();
    ...
    private static final Pattern SENSITIVE_KEY_PATTERN =
            Pattern.compile(".*(password|key|secret|address|url).*", Pattern.CASE_INSENSITIVE);
    ...
    public Map<String, Object> getPublicSettings() {
        Map<String, Object> properties = propertiesProvider.getProperties().entrySet().stream().collect(
                Collectors.toMap(e -> (String) e.getKey(), e -> obfuscateValue((String) e.getKey(), e.getValue())));
    ...
    @ApiResponse(responseCode = "200", description = "returns the list of versions of datashare",
