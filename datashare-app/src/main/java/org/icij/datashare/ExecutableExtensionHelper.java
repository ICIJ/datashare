    ...
    private Path locateExtension() {
        ...
        return switch (extensionBins.size()) {
            case 0 -> throw new RuntimeException(
                    "Couldn't find any executable for extension " + extensionId + " (matching " + extensionPattern +
                    "), extension must be installed !");
            case 1 -> extensionBins.iterator().next().toPath();
            default -> throw new RuntimeException(
                    "Found several executable extension " + extensionId + " (matching " + extensionPattern +
                    "), extension must be installed !");
        };
    }

    private String fileNamePattern() {
        Map.Entry<String, String> res = extractIdVersion(
                ((Extension) this.extensionService.list().stream().filter(ext -> ext.getId().equals(this.extensionId))
                                                  .findAny().orElseThrow(
                                () -> new RuntimeException("couldn't find any extension matching " + extensionId))
                                                  .reference()).url);
