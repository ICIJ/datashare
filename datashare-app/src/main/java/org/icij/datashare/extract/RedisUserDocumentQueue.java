...
public class RedisUserDocumentQueue<T> extends RedisDocumentQueue<T> {
    ...
    @Inject
    public RedisUserDocumentQueue(PropertiesProvider propertiesProvider, RedissonClient redissonClient,
                                  @Assisted String queueName, @Assisted Class<T> clazz) {
        super(redissonClient, queueName,
              Charset.forName(propertiesProvider.get("charset").orElse(Charset.defaultCharset().toString())), clazz);
        this.queueName = queueName;
    }

    public RedisUserDocumentQueue(final User user, PropertiesProvider propertiesProvider, Class<T> clazz) {
        this(propertiesProvider,
             new RedissonClientFactory().withOptions(Options.from(propertiesProvider.getProperties())).create(),
