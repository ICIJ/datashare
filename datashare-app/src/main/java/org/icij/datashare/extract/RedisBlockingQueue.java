...
public class RedisBlockingQueue<T> extends RedissonBlockingQueue<T> implements Closeable {
    ...
    public RedisBlockingQueue(PropertiesProvider propertiesProvider, @Assisted String queueName) {
        this(new RedissonClientFactory().withOptions(Options.from(propertiesProvider.getProperties())).create(),
             queueName);
    }

    public RedisBlockingQueue(RedissonClient redissonClient, String queueName) {
        super(new JsonJacksonCodec(), new CommandSyncService(((Redisson) redissonClient).getConnectionManager(),
                                                             new RedissonObjectBuilder(redissonClient)), queueName,
              redissonClient);
