...
public class BatchScanTask extends DefaultTask<List<Path>> implements Callable<List<Path>> {
    private final BatchHandler<Path> batchHandler;
    private final Task<?> task;
    private final BatchScanVisitor visitor;
    ...
    @Inject
    public BatchScanTask(BatchHandler<Path> batchHandler, @Assisted Task<Long> task,
                         @Assisted Function<Double, Void> updateCallback) {
        this.batchHandler = batchHandler;
        this.task = task;
        Options<String> options = options().createFrom(Options.from(task.args));
        int batchSize = (int) Optional.ofNullable(task.args.get("batchSize"))
    ...
    public List<Path> call() throws IOException {
        visitor.writeBatches(batchHandler, task.id);
        return batchHandler.keys(task.id).toList();
    }
    ...
    public static class BatchScanVisitor extends SimpleFileVisitor<Path> {
        ...
        private final List<List<Path>> batches;
        ...
        public void writeBatches(BatchHandler<Path> batchHandler, String taskId) throws IOException {
            int initialSize = batchHandler.size(taskId);
            ...
            batchHandler.addBatches(taskId, batches.stream());
            this.logger.info("Wrote {} batches !", batchHandler.size(taskId) - initialSize);
        }
