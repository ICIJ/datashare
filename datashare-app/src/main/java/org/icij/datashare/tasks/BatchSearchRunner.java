...
public class BatchSearchRunner implements CancellableTask, UserTask, Callable<BatchSearchRunnerResult> {
    ...
    public BatchSearchRunnerResult call() throws Exception {
        ...
        int maxTimeSeconds =
                parseInt(propertiesProvider.get(BATCH_SEARCH_MAX_TIME_OPT).orElse(DEFAULT_BATCH_SEARCH_MAX_TIME));
        String scrollDuration =
                propertiesProvider.get(BATCH_SEARCH_SCROLL_DURATION_OPT).orElse(DEFAULT_SCROLL_DURATION);
        ...
        if (batchSearch == null) {
            logger.warn(
                    "batch search {} not found in database (check that database url is the same as datashare backend)",
                    taskView.id);
        ...
        try {
            logger.info(
                    "running {} queries for batch search {} on projects {} with throttle {}ms and scroll size of {}",
                    batchSearch.queries.size(), batchSearch.uuid, asCommaConcatNames(batchSearch.projects), throttleMs,
                    scrollSize);
            ...
            for (String s : batchSearch.queries.keySet()) {
                ...
                if (batchSearch.hasQueryTemplate()) { // for retro-compatibility should be removed at some point to keep only bodyTemplate
                    searcher = indexer.search(batchSearch.projects.stream().map(ProjectProxy::getId).collect(toList()),
                                              Document.class, batchSearch.queryTemplate)
                                      .with(batchSearch.fuzziness, batchSearch.phraseMatches).withoutSource("content")
                                      .limit(scrollSize);
                    docsToProcess = searcher.scroll(scrollDuration, query).collect(toList());
                } else {
                    searcher = indexer.search(batchSearch.projects.stream().map(ProjectProxy::getId).collect(toList()),
                                              Document.class, new SearchQuery(query));
                    ((Indexer.QueryBuilderSearcher) searcher).withFieldValues("contentType",
                                                                              batchSearch.fileTypes.toArray(
                                                                                      new String[] {}))
                                                             .withPrefixQuery("path", batchSearch.paths.toArray(
                                                                     new String[] {}))
                ...
                while (!docsToProcess.isEmpty() && numberOfResults < MAX_BATCH_RESULT_SIZE - MAX_SCROLL_SIZE) {
                    ...
                    if (DatashareTime.getInstance().currentTimeMillis() - beforeScrollLoop < maxTimeSeconds * 1000L) {
                        ...
                        throw new SearchException(query, new TimeoutException(
                                "Batch timed out after " + maxTimeSeconds + "s"));
                    }
            ...
            SearchException searchException =
                    new SearchException(query, ElasticSearchAdapterException.createFrom(esEx));
        ...
        if (persisted == null) {
            logger.warn(
                    "batch search {} disappeared before reloading final counters, reporting last known nbQueriesWithoutResults",
                    taskView.id);
