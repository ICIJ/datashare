...
public class CreateNlpBatchesFromIndex extends DefaultTask<List<String>> implements UserTask, CancellableTask {
    ...
    public CreateNlpBatchesFromIndex(
        ...
        this.batchSize = Optional.ofNullable((int) taskView.args.get(NLP_BATCH_SIZE_OPT)).orElse(DEFAULT_NLP_BATCH_SIZE);
        this.maxTextLength = (int)Optional.ofNullable(taskView.args.get(NLP_MAX_TEXT_LENGTH_OPT))
                .orElse(DEFAULT_NLP_MAX_TEXT_LENGTH);
        ...
        this.scrollSize = (int)Optional.ofNullable(taskView.args.get(SCROLL_SIZE_OPT)).orElse(DEFAULT_SCROLL_SIZE);
