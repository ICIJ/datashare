...
public class PluginService extends DeliverableService<Plugin> {
    ...
    public PluginService(PropertiesProvider propertiesProvider, ExtensionService extensionService) {
        this(Paths.get(propertiesProvider.get(PropertiesProvider.PLUGINS_DIR_OPT).orElse("." + PLUGINS_BASE_URL)),
             extensionService);
    }
    ...
    public String addPlugins(String stringContent, List<String> userProjects) {
        File[] dirs = ofNullable(deliverablesDir.toFile().listFiles(File::isDirectory)).orElseThrow(
                () -> new IllegalStateException("invalid path for plugins: " + deliverablesDir));
        String scriptsString = stream(dirs).map(d -> projectFilter(d.toPath(), userProjects)).filter(Objects::nonNull)
                                           .map(this::getPluginUrl).filter(Objects::nonNull)
                                           .map(s -> "<script src=\"" + s + "\" defer></script>").collect(joining());
        String cssString = stream(dirs).map(d -> getCssPluginUrl(d.toPath())).filter(Objects::nonNull)
                                       .map(s -> "<link rel=\"stylesheet\" href=\"" + s + "\">").collect(joining());
        return stringContent.replace("</body>", scriptsString + "</body>").replace("</head>", cssString + "</head>");
    }
    ...
    Path projectFilter(Path pluginDir, List<String> projects) {
        try {
            Path packageJson = pluginDir.resolve("package.json");
            if (packageJson.toFile().isFile()) {
                Map<String, Object> packageMap = new ObjectMapper().readValue(packageJson.toFile(),
                ...
                if (packageMap.containsKey("private")) {
                    ...
                    if (packageMap.containsKey("datashare")) {
                        ...
                        List<String> pluginProjects =
                                ofNullable((List<String>) datashareMap.get("projects")).orElse(Collections.emptyList());
    ...
    private Path getPluginProperty(Path packageJson, String property) {
        try {
            Map<String, Object> packageJsonMap =
                    new ObjectMapper().readValue(packageJson.toFile(), new TypeReference<HashMap<String, Object>>() {});
