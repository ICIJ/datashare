...
public class UsersInDb implements UserStore {
    ...
    public WebResponse<org.icij.datashare.user.User> listUsers(UserFilter filter,
                                                               ...
                                                               int size) {
        Stream<org.icij.datashare.user.User> stream =
                userRepository.listUsers(filter).stream().filter(filter::matches).map(DatashareUser::new);
        if (sort != null)
    ...
    public List<org.icij.datashare.user.User> getUsersByIds(Set<String> ids) {
        return ids.stream().map(userRepository::getUser).filter(Objects::nonNull).map(DatashareUser::new)
