...
public class BatchSearchRunnerIntTest {
    ...
    public void test_search_with_file_types_ok() throws Exception {
        ...
        indexer.add(TEST_INDEXES[0], mydoc);
        BatchSearch search = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("mydoc"), User.local(), false, singletonList("text/plain"), null, 0);
    ...
    public void test_search_with_file_types_ko() throws Exception {
        Document mydoc = createDoc("mydoc").build();
        indexer.add(TEST_INDEXES[0], mydoc);
        BatchSearch searchKo = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("mydoc"), User.local(), false, singletonList("application/pdf"), null, 0);
    ...
    public void test_search_with_paths_ok() throws Exception {
        Document mydoc = createDoc("mydoc").build();
        indexer.add(TEST_INDEXES[0], mydoc);
        BatchSearch searchOk = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("mydoc"), User.local(),false, null,
    ...
    public void test_search_with_paths_ko() throws Exception {
        Document mydoc = createDoc("mydoc").build();
        indexer.add(TEST_INDEXES[0], mydoc);
        BatchSearch searchKo = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("mydoc"), User.local(),false, null,
    ...
    public void test_search_with_fuzziness() throws Exception {
        Document mydoc = createDoc("mydoc").build();
        indexer.add(TEST_INDEXES[0], mydoc);
        BatchSearch searchKo1 = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("doc"), User.local(),false, null,
                null, 1);
        BatchSearch searchKo2 = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("nodoc"), User.local(),false, null,
                null, 1);
        BatchSearch searchOk = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("hedoc"), User.local(),false, null,
                null, 2);
    ...
    public void test_search_with_phraseMatches() throws Exception {
        ...
        indexer.add(TEST_INDEXES[0], mydoc);
        BatchSearch searchKo = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("to find mydoc"), User.local(),false, null,
                null, true);
        BatchSearch searchOk = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("mydoc to find"), User.local(),false, null,
                null,true);
    ...
    public void test_search_with_phraseMatches_with_ner() throws Exception {
        ...
        indexer.add(TEST_INDEXES[0], mydoc);
        indexer.add(TEST_INDEXES[0], NamedEntity.create(NamedEntity.Category.PERSON, "anne", asList(12L), mydoc.getId(), mydoc.getRootDocument(), Pipeline.Type.CORENLP, Language.FRENCH));
        BatchSearch searchKo = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("anne doc"), User.local(),false, null,
                null, true);
        BatchSearch searchOk = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("anne's doc"), User.local(),false, null,
                null,true);
    ...
    public void test_search_phrase_matches_with_slop() throws Exception {
        ...
        indexer.add(TEST_INDEXES[0], mydoc);
        BatchSearch search = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("find mydoc"), User.local(), false, null,
                 null, 2,true);
    ...
    public void test_search_with_operators() throws Exception {
        ...
        indexer.add(TEST_INDEXES[0], mydoc1);
        indexer.add(TEST_INDEXES[0], mydoc2);
        BatchSearch search = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("mydoc AND one"), User.local());
    ...
    public void test_search_with_error() throws Exception {
        ...
        indexer.add(TEST_INDEXES[0], mydoc);
        BatchSearch search = new BatchSearch(project(TEST_INDEXES[0]), "name", "desc", asSet("AND mydoc"), User.local());
