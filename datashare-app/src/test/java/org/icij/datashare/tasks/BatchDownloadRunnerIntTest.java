...
public class BatchDownloadRunnerIntTest {
    ...
    public void test_update_batch_download_zip_size() throws Exception {
        ...
        BatchDownload bd = createBatchDownload("*");
        Task taskView = createTaskView(bd);
        DatashareTaskResult<UriResult> result = new BatchDownloadRunner(indexer, createProvider(), taskView, taskView.progress(taskModifier::progress)).call();

        assertThat(result.value().size()).isGreaterThan(0);
    }
    ...
    private BatchDownload createBatchDownload(String query) {
        return new BatchDownload(List.of(project(TEST_INDEX)), local(), query, null, fs.getRoot().toPath(), false);
    }
