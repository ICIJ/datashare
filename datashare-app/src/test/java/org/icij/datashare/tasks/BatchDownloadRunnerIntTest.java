...
public class BatchDownloadRunnerIntTest {
    ...
    public void test_empty_response() throws Exception {
        ...
        DatashareTask<File> taskView = createTaskView(bd);
    ...
    public void test_progress_rate() throws Exception {
        new IndexerHelper(es.client).indexFile("mydoc.txt", "content", fs);
        DatashareTask<File> taskView = createTaskView(createBatchDownload("*"));
    ...
    private DatashareTask<File> createTaskView(BatchDownload bd) {
        return new DatashareTask<>(BatchDownloadRunner.class.getName(), bd.user, new HashMap<>() {{
