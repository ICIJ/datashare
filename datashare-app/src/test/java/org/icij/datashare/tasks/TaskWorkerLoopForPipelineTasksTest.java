...
public class TaskWorkerLoopForPipelineTasksTest {
    ...
    public void test_scan_task() throws Exception {
        DatashareTask<Long> task = new DatashareTask<>(ScanTask.class.getName(), User.local(), Map.of("dataDir", "/path/to/files"));
    ...
    public void test_index_task() throws Exception {
        DatashareTask<Long> task = new DatashareTask<>(IndexTask.class.getName(), User.local(), new HashMap<>());
    ...
    public void test_extract_nlp_task() throws Exception {
        DatashareTask<Long> task = new DatashareTask<>(ExtractNlpTask.class.getName(), User.local(), Map.of("nlpPipeline", "EMAIL"));
    ...
    public void test_scan_index_task() throws Exception {
        DatashareTask<Long> task = new DatashareTask<>(ScanIndexTask.class.getName(), User.local(), new HashMap<>());
    ...
    public void test_enqueue_from_index_task() throws Exception {
        DatashareTask<Long> task = new DatashareTask<>(EnqueueFromIndexTask.class.getName(), User.local(), Map.of("nlpPipeline", "EMAIL"));
    ...
    public void test_deduplicate_task() throws Exception {
        DatashareTask<Long> task = new DatashareTask<>(DeduplicateTask.class.getName(), User.local(), new HashMap<>());
    ...
    private void testTaskWithTaskRunner(Task<Long> task) throws Exception {
        taskSupplier.startTask(task.name, User.local(), task.getArgs());
