...
public class ProjectResourceTest extends AbstractProdWebServerTest {
    ...
    public void test_update_project_in_server_mode_by_admin() throws URISyntaxException, IOException {
        ...
        configure(routes -> {
            ...
            routes.filter(basicAuthFilter).registerAroundAnnotation(ProjectPolicy.class, projectPolicyAnnotation).add(new PolicyResource(authorizer, repository)).add(projectResource);
        });
