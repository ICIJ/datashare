...
public class EsEmbeddedServerTest {
    ...
    public void launch_with_bad_codec() {
        try {
            ...
            assertThat(logbackCapturingRule.logs(Level.ERROR)).contains("Your index version on disk (data/path) doesn't seem to have the same version as the embedded Elasticsearch engine (7.16.3). Please migrate it with snapshots, or remove it then restart datashare.");
        }
    }
