...
public abstract class AbstractElasticSearchSpewer extends Spewer {
    private static final Logger logger = LoggerFactory.getLogger(AbstractElasticSearchSpewer.class);
    ...
    public AbstractElasticSearchSpewer(FieldNames fields, Indexer indexer, LanguageGuesser languageGuesser,
                                       IndexOptions options) {
        super(fields);
        this.indexer = indexer;
        this.languageGuesser = languageGuesser;
        this.options = options;
        this.indexName = options.defaultIndexName();
    }

    /**
     * Method called after indexing a document
     * @param document
     * @param index
     */
    ...
    protected void writeDocument(TikaDocument doc, TikaDocument parent, TikaDocument root, int level) throws
        ...
        if (parent == null && isDuplicate(doc.getId(), doc.getPath())) {
            ...
            indexer.add(indexName, new Duplicate(doc.getPath(), doc.getId(), options.digestAlgorithm()));
            docType = "Duplicate";
        } else {
            ...
            postIndexation(document, indexName);
        }
    ...
    String readContent(TikaDocument document) throws IOException {
        ...
        final int limit = Math.min(options.maxContentLength() == -1 ? MAX_CONTENT_LENGTH : options.maxContentLength(),
                                   MAX_CONTENT_LENGTH);
    ...
    public AbstractElasticSearchSpewer setManifestRecorder(ManifestRecorder manifestRecorder) {
