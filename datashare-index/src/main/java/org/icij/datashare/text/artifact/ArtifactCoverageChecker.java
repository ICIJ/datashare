...
public class ArtifactCoverageChecker {
    ...
    public record Report(long checked, List<Hole> holes, long empties) {
        ...
        public String summary() {
            StringBuilder builder =
                    new StringBuilder(String.format("checked %d document(s), %d hole(s)%n", checked, holes.size()));
            holes.stream().limit(MAX_HOLES_IN_SUMMARY).forEach(hole -> builder.append(
                    String.format("  %s (root %s): %s%n", hole.docId(), hole.rootId(), hole.reason())));
            if (holes.size() > MAX_HOLES_IN_SUMMARY) {
                builder.append(String.format("  ... and %d more%n", holes.size() - MAX_HOLES_IN_SUMMARY));
            }
            if (empties > 0) {
                builder.append(
                        String.format("%d empty embed(s) (present, zero-byte, not counted as holes)%n", empties));
            }
    ...
    public Report check(Project project, Path artifactDir, Stream<Document> documents) {
        ...
        while (iterator.hasNext()) {
            ...
            try {
                ManifestEntry raw =
                        manifests.get(ArtifactPath.dir(projectRoot, document.getId()), ArtifactType.RAW.token());
    ...
    public Report check(Project project, Path artifactDir, int scrollSize) throws Exception {
        Indexer.Searcher searcher =
                indexer.search(List.of(project.name), Document.class).withoutSource("content", "content_translated")
                       .limit(scrollSize);
        long checked = 0;
        long empties = 0;
        List<Hole> holes = new ArrayList<>();
