...
class ElasticsearchQueryBuilderSearcher extends ElasticsearchSearcher implements Indexer.QueryBuilderSearcher {
    ...
    ElasticsearchQueryBuilderSearcher(ElasticsearchClient client, final List<String> indexesNames,
                                      final Class<? extends Entity> cls) {
        super(client, indexesNames, cls, null);
        this.boolQueryBuilder = new BoolQuery.Builder().must(
                must -> must.match(m -> m.field("type").query(JsonObjectMapper.getType(cls))));
        this.stringQuery = null;
    }

    ElasticsearchQueryBuilderSearcher(ElasticsearchClient client, final List<String> indexesNames,
                                      final Class<? extends Entity> cls, SearchQuery query) {
        super(client, indexesNames, cls, null);
        this.boolQueryBuilder = new BoolQuery.Builder().must(
                must -> must.match(m -> m.field("type").query(JsonObjectMapper.getType(cls))));
    ...
    protected BoolQuery.Builder getBoolQueryBuilder(String query) {
        return query != null ? boolQueryBuilder.must(m -> m.matchAll(ma -> ma)).must(m -> m.queryString(
                qs -> qs.query(buildQueryString(query, fuzziness, phraseMatches)))) : boolQueryBuilder;
    }
    ...
    public Indexer.QueryBuilderSearcher without(Pipeline.Type... nlpPipelines) {
        this.boolQueryBuilder.mustNot(q -> q.constantScore(cs -> cs.filter(qt -> qt.terms(t -> t.field("nerTags")
                                                                                                .terms(tqf -> tqf.value(
                                                                                                        stream(nlpPipelines).map(
    ...
    public Indexer.QueryBuilderSearcher with(Pipeline.Type... nlpPipelines) {
        this.boolQueryBuilder.must(q -> q.constantScore(cs -> cs.filter(qt -> qt.terms(t -> t.field("nerTags")
                                                                                             .terms(tqf -> tqf.value(
                                                                                                     stream(nlpPipelines).map(
                                                                                                                                 Pipeline.Type::toString)
                                                                                                                         .map(FieldValue::of)
    ...
    public Indexer.QueryBuilderSearcher with(Tag... tags) {
        this.boolQueryBuilder.must(q -> q.constantScore(cs -> cs.filter(qt -> qt.terms(t -> t.field("tags")
                                                                                             .terms(tqf -> tqf.value(
                                                                                                     stream(tags).map(
                                                                                                                         tag -> FieldValue.of(
                                                                                                                                 tag.label))
    ...
    public Indexer.QueryBuilderSearcher withFieldValues(String key, String... values) {
        if (values.length > 0) {
            this.boolQueryBuilder.must(m -> m.terms(
                    t -> t.field(key).terms(tqf -> tqf.value(stream(values).map(FieldValue::of).collect(toList())))));
        }
        return this;
    }
