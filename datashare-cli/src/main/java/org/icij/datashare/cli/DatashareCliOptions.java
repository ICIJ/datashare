...
public final class DatashareCliOptions {
    ...
    private static final Path DEFAULT_DATASHARE_HOME =
            Paths.get(System.getProperty("user.home"), ".local/share/datashare");
    private static final Integer DEFAULT_NLP_PARALLELISM = 1;
    public static final Integer DEFAULT_PARALLELISM =
    ...
    public static final String DEFAULT_DATA_DIR =
            Paths.get(System.getProperty("user.home")).resolve("Datashare").toString();
    ...
    public static final String DEFAULT_DATA_SOURCE_URL =
            "jdbc:sqlite:file:" + DEFAULT_DATASHARE_HOME.resolve("dist/datashare.db");
    public static final String DEFAULT_DEFAULT_PROJECT = "local-datashare";
    public static final String DEFAULT_ELASTICSEARCH_ADDRESS =
            EnvUtils.resolveUri("elasticsearch", "http://elasticsearch:9200");
    ...
    public static final String DEFAULT_ELASTICSEARCH_SETTINGS =
            DEFAULT_DATASHARE_HOME.resolve("elasticsearch.yml").toString();
    ...
    public static final Map<String, String> OPT_ALIASES = Map.ofEntries(Map.entry(PORT_OPT, TCP_LISTEN_PORT_OPT));

    static void bindHost(OptionParser parser) {
        parser.acceptsAll(asList(BIND_HOST_ABBR_OPT, BIND_HOST_OPT),
    ...
    static void stages(OptionParser parser) {
        parser.acceptsAll(singletonList(PipelineHelper.STAGES_OPT),
                          format("Stages to be run separated by %s %s", PipelineHelper.STAGES_SEPARATOR,
                                 Arrays.toString(Stage.values()))).withRequiredArg().ofType(String.class);
    }

    static void mode(OptionParser parser) {
        parser.acceptsAll(asList(MODE_ABBR_OPT, MODE_OPT), "Datashare run mode " + Arrays.toString(Mode.values()))
              .withRequiredArg().ofType(Mode.class).defaultsTo(DEFAULT_MODE);
    }

    static void charset(OptionParser parser) {
        parser.acceptsAll(singletonList(CHARSET_OPT), "Datashare default charset. Example: " + Arrays.toString(
                      new Charset[] {StandardCharsets.UTF_8, StandardCharsets.ISO_8859_1})).withRequiredArg()
              .defaultsTo(DEFAULT_CHARSET);
    }

    static void defaultUser(OptionParser parser) {
        parser.acceptsAll(asList(DEFAULT_USER_NAME_ABBR_OPT, DEFAULT_USER_NAME_OPT), "Default local user name")
              .withRequiredArg().ofType(String.class).defaultsTo(DEFAULT_USER);
    }

    static void defaultUserProjectKey(OptionParser parser) {
        parser.acceptsAll(List.of(OAUTH_USER_PROJECTS_KEY_OPT),
                          "Json field name sent by the Identity Provider that contains user projects.")
              .withRequiredArg().ofType(String.class).defaultsTo(User.DEFAULT_PROJECTS_KEY);
    }

    static void followSymlinks(OptionParser parser) {
        parser.acceptsAll(singletonList(FOLLOW_SYMLINKS_OPT), "Follow symlinks while scanning documents")
              .withRequiredArg().ofType(Boolean.class).defaultsTo(DEFAULT_FOLLOW_SYMLINKS);
    }

    static void cors(OptionParser parser) {
        parser.acceptsAll(singletonList(CORS_OPT), "CORS headers (needs the web option)").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_CORS);
    }

    static void settings(OptionParser parser) {
        parser.acceptsAll(asList(SETTING_ABBR_OPT, SETTINGS_OPT), "Property settings file").withRequiredArg()
              .ofType(String.class);
    }

    static void pluginsDir(OptionParser parser) {
        parser.acceptsAll(singletonList(PLUGINS_DIR_OPT), "Plugins directory").withRequiredArg().ofType(String.class)
              .defaultsTo(DEFAULT_PLUGIN_DIR);
    }

    static void pluginList(OptionParser parser) {
        parser.acceptsAll(singletonList(PLUGIN_LIST_OPT), "Plugins list matching provided string").withOptionalArg()
              .ofType(String.class);
    }

    static void pluginInstall(OptionParser parser) {
        parser.acceptsAll(singletonList(PLUGIN_INSTALL_OPT),
                          "Install plugin with either id or URL or file path (needs pluginsDir option)")
              .withRequiredArg().ofType(String.class);
    }

    static void pluginDelete(OptionParser parser) {
        parser.acceptsAll(singletonList(PLUGIN_DELETE_OPT),
                          "Delete plugin with its id or base directory (needs pluginsDir option)").withRequiredArg()
              .ofType(String.class);
    }
    ...
    static void extensionInstall(OptionParser parser) {
        parser.acceptsAll(singletonList(EXTENSION_INSTALL_OPT),
                          "Install extension with either id or URL or file path (needs extensionsDir option)")
              .withRequiredArg().ofType(String.class);
    }

    static void extensionDelete(OptionParser parser) {
        parser.acceptsAll(singletonList(EXTENSION_DELETE_OPT),
                          "Delete extension with its id or base directory (needs extensionsDir option)")
              .withRequiredArg().ofType(String.class);
    }

    public static void extensionsDir(OptionParser parser) {
        parser.acceptsAll(singletonList(EXTENSIONS_DIR_OPT), "Extensions directory (backend)").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_EXTENSIONS_DIR);
    }

    static void tcpListenPort(OptionParser parser) {
        parser.acceptsAll(List.of(PORT_OPT, TCP_LISTEN_PORT_OPT), "Port used by the HTTP server").withRequiredArg()
              .ofType(Integer.class).defaultsTo(DEFAULT_TCP_LISTEN_PORT);
    }

    static void sessionSigningKey(OptionParser parser) {
        parser.acceptsAll(singletonList(SESSION_SIGNING_KEY_OPT),
                          "HMAC key used to sign session IDs (defaults to oauthClientSecret)").withRequiredArg()
              .ofType(String.class);
    }

    static void sessionTtlSeconds(OptionParser parser) {
        parser.acceptsAll(singletonList(SESSION_TTL_SECONDS_OPT), "Time to live for a HTTP session in seconds")
              .withRequiredArg().ofType(Integer.class).defaultsTo(DEFAULT_SESSION_TTL_SECONDS);
    }

    static void protectedUriPrefix(OptionParser parser) {
        parser.acceptsAll(singletonList(PROTECTED_URI_PREFIX_OPT), "Protected URI prefix").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_PROTECTED_URI_PREFIX);
    }
    ...
    static void createIndex(OptionParser parser) {
        parser.acceptsAll(singletonList(CREATE_INDEX_OPT), "creates an index with the given name").withRequiredArg()
              .ofType(String.class);
    }

    static void indexTimeout(OptionParser parser) {
        parser.acceptsAll(singletonList(INDEX_TIMEOUT_OPT),
                          "Time to wait in minutes before consumer termination during document indexing (Default 30m)")
              .withRequiredArg().withValuesConvertedBy(new PositiveIntegerConverter())
              .defaultsTo(DEFAULT_INDEX_TIMEOUT);
    }

    static void genApiKey(OptionParser parser) {
        parser.acceptsAll(asList(CRE_API_KEY_ABBR_OPT, CRE_API_KEY_OPT),
                          "Generate and store api key for user defaultUser (see opt)").withRequiredArg()
              .ofType(String.class);
    }

    static void getApiKey(OptionParser parser) {
        parser.acceptsAll(singletonList(GET_API_KEY_OPT), "existing api key for user").withRequiredArg()
              .ofType(String.class);
    }

    static void delApiKey(OptionParser parser) {
        parser.acceptsAll(singletonList(DEL_API_KEY_OPT), "Delete api key for user").withRequiredArg()
              .ofType(String.class);
    }
    ...
    static void dataDir(OptionParser parser) {
        parser.acceptsAll(asList(DATA_DIR_ABBR_OPT, DATA_DIR_OPT), "Document source files directory").withRequiredArg()
              .ofType(File.class).defaultsTo(new File(DEFAULT_DATA_DIR));
    }
    ...
    static void artifacts(OptionParser parser) {
        parser.acceptsAll(List.of(ARTIFACTS_OPT),
                          "Artifact types to produce (comma-separated, e.g. " + ArtifactType.tokens() +
                          "). Bare flag = all types. " +
    ...
    static void artifactsForce(OptionParser parser) {
        parser.acceptsAll(List.of(ARTIFACTS_FORCE_OPT),
    ...
    static void rootHost(OptionParser parser) {
        parser.acceptsAll(singletonList(ROOT_HOST_OPT), "Datashare host for urls").withRequiredArg();
    }

    static void messageBusAddress(OptionParser parser) {
        parser.acceptsAll(singletonList(MESSAGE_BUS_OPT), "Message bus address").withRequiredArg()
              .defaultsTo(DEFAULT_MESSAGE_BUS_ADDRESS);
    }

    public static void busType(OptionParser parser) {
        parser.acceptsAll(singletonList(BUS_TYPE_OPT), "Backend data bus type.").withRequiredArg()
              .ofType(QueueType.class).defaultsTo(DEFAULT_BUS_TYPE);
    }

    public static void taskRoutingStrategy(OptionParser parser) {
        parser.acceptsAll(singletonList(TASK_ROUTING_STRATEGY_OPT),
                          format("Task Manager routing strategy (%s).", Arrays.toString(RoutingStrategy.values())))
              .withRequiredArg().ofType(RoutingStrategy.class).defaultsTo(DEFAULT_TASK_ROUTING_STRATEGY);
    }

    static void taskRoutingKey(OptionParser parser) {
        parser.acceptsAll(singletonList(TASK_ROUTING_KEY_OPT),
    ...
    static void redisAddress(OptionParser parser) {
        parser.acceptsAll(singletonList(REDIS_ADDRESS_OPT), "Redis queue address").withRequiredArg()
              .defaultsTo(DEFAULT_REDIS_ADDRESS);
    }

    static void queueName(OptionParser parser) {
        parser.acceptsAll(singletonList(QUEUE_NAME_OPT), "Extract queue name").withRequiredArg().ofType(String.class)
              .defaultsTo(DEFAULT_QUEUE_NAME);
    }

    static void queueType(OptionParser parser) {
        parser.acceptsAll(singletonList(QUEUE_TYPE_OPT), "Backend queues and sets type.").withRequiredArg()
              .ofType(QueueType.class).defaultsTo(DEFAULT_QUEUE_TYPE);
    }

    static void queueCapacity(OptionParser parser) {
        parser.acceptsAll(singletonList(QUEUE_CAPACITY_OPT),
                          "Queue capacity is the size of the internal file path buffer used by the queue.")
              .withRequiredArg().ofType(Integer.class).withValuesConvertedBy(new PositiveIntegerConverter())
              .defaultsTo(DEFAULT_QUEUE_CAPACITY);
    }

    static void fileParserParallelism(OptionParser parser) {
        parser.acceptsAll(asList(PARSER_PARALLELISM_ABBR_OPT, PARSER_PARALLELISM_OPT), "Number of file parser threads.")
              .withRequiredArg().ofType(Integer.class).defaultsTo(DEFAULT_PARSER_PARALLELISM);
    }

    static void nlpParallelism(OptionParser parser) {
        parser.acceptsAll(asList(NLP_PARALLELISM_ABBR_OPT, NLP_PARALLELISM_OPT),
                          "Number of NLP extraction threads per pipeline.").withRequiredArg().ofType(Integer.class)
              .defaultsTo(DEFAULT_NLP_PARALLELISM);
    }

    static void nlpBatchSize(OptionParser parser) {
        parser.acceptsAll(List.of(NLP_BATCH_SIZE_OPT), "Batch size of NLP extraction task in number of documents.")
              .withRequiredArg().ofType(Integer.class).defaultsTo(DEFAULT_NLP_BATCH_SIZE);
    }
    ...
    public static void batchSearchMaxTime(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_SEARCH_MAX_TIME_OPT), "Max time for batch search in seconds")
              .withRequiredArg().ofType(Integer.class);
    }

    public static void batchThrottle(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_THROTTLE_OPT), "Throttle for batch in milliseconds").withRequiredArg()
              .ofType(Integer.class);
    }

    public static void scroll(OptionParser parser) {
        parser.acceptsAll(singletonList(SCROLL_DURATION_OPT),
                          "Scroll duration used for elasticsearch scrolls (SCANIDX task)").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_SCROLL_DURATION);
    }

    public static void scrollSize(OptionParser parser) {
        parser.acceptsAll(singletonList(SCROLL_SIZE_OPT), "Scroll size used for elasticsearch scrolls (SCANIDX task)")
              .withRequiredArg().ofType(Integer.class).defaultsTo(DEFAULT_SCROLL_SIZE);
    }

    public static void scrollSlices(OptionParser parser) {
        parser.acceptsAll(singletonList(SCROLL_SLICES_OPT),
                          "Scroll slice max number used for elasticsearch scrolls (SCANIDX task)").withRequiredArg()
              .ofType(Integer.class).defaultsTo(DEFAULT_SCROLL_SLICES);
    }

    public static void batchSearchScroll(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_SEARCH_SCROLL_DURATION_OPT),
                          "Scroll duration used for elasticsearch scrolls (Batch Search)").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_SCROLL_DURATION);
    }

    public static void batchSearchScrollSize(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_SEARCH_SCROLL_SIZE_OPT),
                          "Scroll size used for elasticsearch scrolls (Batch Search)").withRequiredArg()
              .ofType(Integer.class).defaultsTo(DEFAULT_SCROLL_SIZE);
    }

    public static void batchDownloadScroll(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_DOWNLOAD_SCROLL_DURATION_OPT),
                          "Scroll duration used for elasticsearch scrolls (Batch Download)").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_SCROLL_DURATION);
    }

    public static void batchDownloadScrollSize(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_DOWNLOAD_SCROLL_SIZE_OPT),
                          "Scroll size used for elasticsearch scrolls (Batch Download)").withRequiredArg()
              .ofType(Integer.class).defaultsTo(DEFAULT_SCROLL_SIZE);
    }

    public static void redisPoolSize(OptionParser parser) {
        parser.acceptsAll(singletonList(REDIS_POOL_SIZE_OPT),
                          "Pool size for the main Redis client. When unset it defaults to parallelism + " +
                          REDIS_POOL_SIZE_OVERHEAD +
                          " so blocking workers never starve the pool; an explicit value below that floor is raised to it.")
              .withRequiredArg().ofType(Integer.class);
    }

    public static void elasticsearchDataPath(OptionParser parser) {
        parser.acceptsAll(singletonList(ELASTICSEARCH_DATA_PATH_OPT), "Data path used for embedded Elasticsearch")
              .withRequiredArg().ofType(String.class).defaultsTo(DEFAULT_ELASTICSEARCH_DATA_PATH);
    }

    public static void elasticsearchPath(OptionParser parser) {
        parser.acceptsAll(singletonList(ELASTICSEARCH_PATH_OPT),
                          "Path used for launching Elasticsearch (should be installed with official tar/zip).")
              .withRequiredArg().ofType(String.class).defaultsTo(DEFAULT_ELASTICSEARCH_PATH);
    }

    public static void elasticsearchSettings(OptionParser parser) {
        parser.acceptsAll(singletonList(ELASTICSEARCH_SETTINGS_OPT),
                          "Path to elasticsearch.yml settings file for embedded Elasticsearch (optional, loaded if exists)")
              .withRequiredArg().ofType(String.class).defaultsTo(DEFAULT_ELASTICSEARCH_SETTINGS);
    }

    public static void elasticsearchMaxIdleConnectionTime(OptionParser parser) {
        parser.acceptsAll(singletonList(ELASTICSEARCH_MAX_IDLE_CONNECTION_TIME_OPT),
                          "Max idling time for a TCP connection to ES to be reused").withRequiredArg()
              .ofType(Integer.class).defaultsTo(DEFAULT_ELASTICSEARCH_MAX_IDLE_CONNECTION_TIME);
    }

    public static void reportName(OptionParser parser) {
        parser.acceptsAll(singletonList(REPORT_NAME_OPT),
                          "name of the map for the report map (where index results are stored). " +
                          "No report records are saved if not provided").withRequiredArg().ofType(String.class);
    }

    static void enableBrowserOpenLink(OptionParser parser) {
        parser.acceptsAll(singletonList(BROWSER_OPEN_LINK_OPT), "try to open link in the default browser")
              .withRequiredArg().ofType(Boolean.class).defaultsTo(DEFAULT_BROWSER_OPEN_LINK);
    }

    static void enableOcr(OptionParser parser) {
        parser.acceptsAll(asList(OCR_ABBR_OPT, OCR_OPT), "Run optical character recognition at file parsing time. " +
                                                         "(Tesseract must be installed beforehand).").withRequiredArg()
              .ofType(Boolean.class).defaultsTo(DEFAULT_OCR);
    }

    static void language(OptionParser parser) {
        parser.acceptsAll(asList(LANGUAGE_ABBR_OPT, LANGUAGE_OPT),
    ...
    static void ocrType(OptionParser parser) {
        parser.acceptsAll(List.of(OCR_TYPE_OPT), "OCR implementation: TESSERACT or TESS4J").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_OCR_TYPE);
    }

    static void ocrTimeout(OptionParser parser) {
        parser.acceptsAll(List.of(OCR_TIMEOUT), "OCR timeout").withRequiredArg().ofType(String.class)
    ...
    static void parseTimeout(OptionParser parser) {
        parser.acceptsAll(List.of(PARSE_TIMEOUT_OPT),
                          ...
                          "Set to 0 to disable. Defaults to 24h.").withRequiredArg().ofType(String.class)
              .defaultsTo(DEFAULT_PARSE_TIMEOUT);
    }

    static void ocrStrategy(OptionParser parser) {
        parser.acceptsAll(List.of(OCR_STRATEGY_OPT),
    ...
    static void maxEmbedDepth(OptionParser parser) {
        parser.acceptsAll(List.of(MAX_EMBED_DEPTH_OPT), MAX_EMBED_DEPTH_DESC).withRequiredArg().ofType(Integer.class)
              .defaultsTo(DEFAULT_MAX_EMBED_DEPTH);
    }

    static void nlpPipeline(OptionParser parser) {
        parser.acceptsAll(asList(NLP_PIPELINE_ABBR_OPT, NLP_PIPELINE_OPT), "NLP pipeline to be run.").withRequiredArg()
              .defaultsTo(DEFAULT_NLP_PIPELINE);
    }

    static void parallelism(OptionParser parser) {
        parser.acceptsAll(singletonList(PARALLELISM_OPT), "Number of threads allocated for task management.")
              .withRequiredArg().ofType(Integer.class).defaultsTo(DEFAULT_PARALLELISM);
    }

    static void esHost(OptionParser parser) {
        parser.acceptsAll(singletonList(ELASTICSEARCH_ADDRESS_OPT), "Elasticsearch host address").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_ELASTICSEARCH_ADDRESS);
    }

    static void dataSourceUrl(OptionParser parser) {
        parser.acceptsAll(singletonList(DATA_SOURCE_URL_OPT),
                          "Datasource URL. For using memory you can use 'jdbc:sqlite:file:memorydb.db?mode=memory&cache=shared'")
              .withRequiredArg().ofType(String.class).defaultsTo(DEFAULT_DATA_SOURCE_URL);
    }

    static void defaultProject(OptionParser parser) {
        parser.acceptsAll(asList(DEFAULT_PROJECT_ABBR_OPT, DEFAULT_PROJECT_OPT), "Default project name")
              .withRequiredArg().ofType(String.class).defaultsTo(DEFAULT_DEFAULT_PROJECT);
    }

    static void clusterName(OptionParser parser) {
        parser.acceptsAll(singletonList(CLUSTER_NAME_OPT), "Cluster name").withRequiredArg().ofType(String.class)
    ...
    static void authUsersProvider(OptionParser parser) {
        parser.acceptsAll(singletonList(AUTH_USERS_PROVIDER_OPT),
                          "Server mode users provider: database, redis, or a fully-qualified class name (default: database)")
              .withRequiredArg().ofType(String.class);
    }

    static void authFilter(OptionParser parser) {
        parser.acceptsAll(singletonList(AUTH_FILTER_OPT), "Server mode auth filter class").withRequiredArg()
              .ofType(String.class);
    }

    static void oauthSecret(OptionParser parser) {
        parser.acceptsAll(singletonList(OAUTH_CLIENT_SECRET_OPT), "OAuth2 client secret key").withRequiredArg()
              .ofType(String.class);
    }

    static void oauthClient(OptionParser parser) {
        parser.acceptsAll(singletonList(OAUTH_CLIENT_ID_OPT), "OAuth2 client id").withRequiredArg()
              .ofType(String.class);
    }

    static void oauthAuthorizeUrl(OptionParser parser) {
        parser.acceptsAll(singletonList(OAUTH_AUTHORIZE_URL_OPT), "OAuth2 authorize url").withRequiredArg()
              .ofType(String.class);
    }

    static void oauthTokenUrl(OptionParser parser) {
        parser.acceptsAll(singletonList(OAUTH_TOKEN_URL_OPT), "OAuth2 token url").withRequiredArg()
              .ofType(String.class);
    }

    static void oauthApiUrl(OptionParser parser) {
        parser.acceptsAll(singletonList(OAUTH_API_URL_OPT), "OAuth2 api url").withRequiredArg().ofType(String.class);
    }

    static void oauthCallbackPath(OptionParser parser) {
        parser.acceptsAll(singletonList(OAUTH_CALLBACK_PATH_OPT), "OAuth2 callback path (in datashare)")
              .withRequiredArg().ofType(String.class);
    }

    static void oauthDefaultProject(OptionParser parser) {
        parser.acceptsAll(singletonList(OAUTH_DEFAULT_PROJECT_OPT), "Default project to use for Oauth2 users")
              .withRequiredArg().ofType(String.class);
    }

    static void oauthScope(OptionParser parser) {
        parser.acceptsAll(singletonList(OAUTH_SCOPE_OPT), "Set scope in oauth2 callback url, needed for OIDC providers")
              .withRequiredArg().ofType(String.class);
    }

    public static void batchQueueType(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_QUEUE_TYPE_OPT), "").withRequiredArg().ofType(QueueType.class)
              .defaultsTo(DEFAULT_BATCH_QUEUE_TYPE);
    }

    public static void batchDownloadTimeToLive(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_DOWNLOAD_ZIP_TTL_OPT),
                          "Time to live in hour for batch download zip files (Default 24)").withRequiredArg()
              .ofType(Integer.class).defaultsTo(DEFAULT_BATCH_DOWNLOAD_ZIP_TTL);
    }

    public static void batchDownloadMaxNbFiles(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_DOWNLOAD_MAX_NB_FILES_OPT),
                          "Maximum file number that can be archived in a zip (Default 10,000)").withRequiredArg()
              .ofType(Integer.class).defaultsTo(DEFAULT_BATCH_DOWNLOAD_MAX_NB_FILES);
    }

    public static void batchDownloadEncrypt(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_DOWNLOAD_ENCRYPT_OPT),
                          "Whether Batch download zip files are encrypted or not. SmtpUrl should be set to send the password. (default false)")
              .withRequiredArg().ofType(Boolean.class);
    }

    public static void smtpUrl(OptionParser parser) {
        parser.acceptsAll(singletonList(SMTP_URL_OPT),
                          "Smtp url to allow datashare to send emails (ex: smtp://localhost:25)").withRequiredArg()
              .ofType(URI.class);
    }

    public static void embeddedDocumentDownloadMaxSize(OptionParser parser) {
        parser.acceptsAll(singletonList(EMBEDDED_DOCUMENT_DOWNLOAD_MAX_SIZE_OPT),
                          "Maximum download size of embedded documents. Human readable suffix K/M/G for KB/MB/GB (Default 1G)")
              .withRequiredArg().withValuesConvertedBy(regex("[0-9]+[KMG]?"))
              .defaultsTo(DEFAULT_EMBEDDED_DOCUMENT_DOWNLOAD_MAX_SIZE);
    }

    public static void batchDownloadMaxSize(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_DOWNLOAD_MAX_SIZE_OPT),
                          "Maximum total files size that can be zipped. Human readable suffix K/M/G for KB/MB/GB (Default 100M)")
              .withRequiredArg().withValuesConvertedBy(regex("[0-9]+[KMG]?"))
              .defaultsTo(DEFAULT_BATCH_DOWNLOAD_MAX_SIZE);
    }

    public static void batchDownloadDir(OptionParser parser) {
        parser.acceptsAll(singletonList(BATCH_DOWNLOAD_DIR_OPT),
                          "Directory where Batch Download archives are downloaded.").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_BATCH_DOWNLOAD_DIR)
              .withValuesConvertedBy(DatashareCliOptions.toAbsolute());
    }

    public static void maxContentLength(OptionParser parser) {
        parser.acceptsAll(singletonList(MAX_CONTENT_LENGTH_OPT),
                          "Maximum length (in bytes) of extracted text that could be indexed " +
                          "(-1 means no limit and value should be less or equal than 2G). Human readable suffix K/M/G for KB/MB/GB (Default 20M)")
              .withRequiredArg().defaultsTo(DEFAULT_MAX_CONTENT_LENGTH).withValuesConvertedBy(regex("[0-9]+[KMG]?"));
    }

    public static void sessionStoreType(OptionParser parser) {
        parser.acceptsAll(singletonList(SESSION_STORE_TYPE_OPT), "Type of session store").withRequiredArg()
              .ofType(QueueType.class).defaultsTo(DEFAULT_SESSION_STORE_TYPE);
    }

    public static void statusAllowedNets(OptionParser parser) {
        parser.acceptsAll(singletonList(STATUS_ALLOWED_NETS_OPT),
                          "Comma-separated CIDR list for unauthenticated access to /api/status").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_STATUS_ALLOWED_NETS);
    }

    public static void digestMethod(OptionParser parser) {
        parser.acceptsAll(singletonList(DIGEST_ALGORITHM_OPT)).withRequiredArg().ofType(DigestAlgorithm.class)
              .defaultsTo(DEFAULT_DIGEST_METHOD).withValuesConvertedBy(new DigestAlgorithm.DigestAlgorithmConverter());
    }

    public static void digestProjectName(OptionParser parser) {
        parser.acceptsAll(singletonList(DIGEST_PROJECT_NAME_OPT),
                          "Includes the project name in the hash of documents when indexing. " +
    ...
    public static void noDigestProject(OptionParser parser) {
        parser.acceptsAll(singletonList(NO_DIGEST_PROJECT_OPT),
                          "Disable the project name in document hash processing (only using binary contents).")
              .withRequiredArg().ofType(Boolean.class).defaultsTo(DEFAULT_NO_DIGEST_PROJECT);
    }

    public static OptionSpec<String> extOption(OptionParser parser) {
        return parser.acceptsAll(singletonList(EXT_OPT), "Run CLI extension").withRequiredArg().ofType(String.class);
    }

    static void logLevel(OptionParser parser) {
        parser.acceptsAll(singletonList(LOG_LEVEL_OPT),
                          format("Sets the log level of Datashare (%s)", Arrays.toString(Level.values())))
              .withRequiredArg().ofType(String.class).defaultsTo(DEFAULT_LOG_LEVEL);
    }
    ...
    public static void pollingInterval(OptionParser parser) {
        parser.acceptsAll(singletonList(POLLING_INTERVAL_SECONDS_OPT),
                          "in-memory task manager only (ignored with a Redis or AMQP task manager): base interval in seconds for waiting on task worker termination, the window is twice this value")
              .withRequiredArg().ofType(String.class).defaultsTo(DEFAULT_POLLING_INTERVAL_SEC);
    }

    public static void taskRepositoryType(OptionParser parser) {
        parser.acceptsAll(singletonList(TASK_REPOSITORY_OPT),
                          format("type of task repository (%s)", Arrays.toString(TaskRepositoryType.values())))
              .withRequiredArg().ofType(TaskRepositoryType.class).defaultsTo(TaskRepositoryType.DATABASE);
    }

    static void taskManagerPollingInterval(OptionParser parser) {
        parser.acceptsAll(singletonList(TASK_MANAGER_POLLING_INTERVAL_OPT),
                          "Time to wait for task manager polling in milliseconds.").withRequiredArg()
              .ofType(Integer.class).defaultsTo(DEFAULT_TASK_MANAGER_POLLING_INTERVAL);
    }

    static void policyReloadInterval(OptionParser parser) {
        parser.acceptsAll(singletonList(POLICY_RELOAD_INTERVAL_OPT),
                          "Interval in milliseconds to reload Casbin policies from DB. In non-Redis mode defaults to 30s; in Redis mode defaults to 0 (event-driven only). Set to 0 to disable.")
              .withRequiredArg().ofType(Integer.class);
    }

    static void taskProgressUpdateInterval(OptionParser parser) {
        parser.acceptsAll(singletonList(TASK_PROGRESS_INTERVAL_OPT),
                          "Minimum interval between progress updates for the same task in seconds.").withRequiredArg()
              .ofType(Double.class).defaultsTo(DEFAULT_TASK_PROGRESS_INTERVAL_SECONDS);
    }

    static void taskWorkers(OptionParser parser) {
        parser.acceptsAll(singletonList(TASK_WORKERS_OPT), "Number of task workers (threads).").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_TASK_WORKERS);
    }

    static void temporalNamespaceOpt(OptionParser parser) {
        parser.acceptsAll(singletonList(TEMPORAL_NAMESPACE_OPT), "temporal namespace").withRequiredArg()
              .ofType(String.class).defaultsTo(DEFAULT_TEMPORAL_NAMESPACE);
    }

    static void temporalAddressOpt(OptionParser parser) {
        parser.acceptsAll(List.of(TEMPORAL_ADDRESS_OPT), "Temporal address").withRequiredArg().ofType(String.class)
