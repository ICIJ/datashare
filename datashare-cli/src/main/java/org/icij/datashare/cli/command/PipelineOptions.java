...
public class PipelineOptions {
    ...
    @Option(names = {"--artifacts"}, arity = "0..1", fallbackValue = "true", description =
            "Artifact types to produce, comma-separated (bare flag = all types); unknown types are rejected. " +
    ...
    @Option(names = {"--parseTimeout"},
            description = "Wall-clock timeout for a single document's parse and output, e.g. \"30m\" or \"24h\". Set to 0 to disable. Defaults to 24h.",
            defaultValue = DEFAULT_PARSE_TIMEOUT)
    String parseTimeout;
    ...
    @Option(names = {"--ocrStrategy"},
            description = "PDF OCR strategy: NO_OCR (default), AUTO, OCR_AND_TEXT_EXTRACTION, OCR_ONLY. Rendering strategies OCR whole pages for scanned/MRC PDFs; only applies when OCR is enabled.")
    ...
    boolean resume;
    @Option(names = {"--followSymlinks"}, description = "Follow symlinks while scanning (default: ${DEFAULT-VALUE})",
            arity = "1", defaultValue = "true")
    ...
    int scrollSize;
    @Option(names = {"--scrollSlices"}, description = "Scroll slice max number used for elasticsearch scrolls",
            defaultValue = "1")
    int scrollSlices;
    @Option(names = {"--reportName"},
            description = "Name of the map for the report map (where index results are stored). No report records are saved if not provided")
    String reportName;
    @Option(names = {"--maxContentLength"},
            description = "Maximum length (in bytes) of extracted text that could be indexed",
