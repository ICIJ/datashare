...
@Command(name = "grant", mixinStandardHelpOptions = true,
        description = {"Grant a role to a user on a project (replaces any existing role).", "", "Examples:",
...
public class ProjectGrantCommand implements Runnable, DatashareSubcommand {
    ...
    public void run() {
        try {
            ...
            if (project != null)
                Validators.projectName(project);
            if (user != null)
                Validators.login(user);
            if (roleIn != null)
                Validators.projectRole(roleIn);

            Prompter prompter = resolvePrompter();
            if (project == null)
                project = require(prompter, "Project name", Validators::projectName, "--project");
            if (user == null)
                user = require(prompter, "User login", Validators::login, "--user");
            // Validators.projectRole returns Role; wrap as Consumer for the require() signature.
            // We only care that it doesn't throw.
            if (roleIn == null)
                roleIn = require(prompter, "Role (admin|editor|member|visitor)", s -> Validators.projectRole(s),
                                 "--role");
